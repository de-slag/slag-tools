...
echo
echo "@reboot    for f in /root/logic/at-reboot/*.sh; do bash "$f" -H ;done >> /root/logs/at-reboot.log 2>&1"
echo "* * * * *  for f in /root/logic/minutely/*.sh; do bash "$f" -H ;done >> /root/logs/minutely.log 2>&1"
echo "0 5 * * *  for f in /root/logic/daily/*.sh; do bash "$f" -H ;done >> /root/logs/daily.log 2>&1"
echo "0 0 * * 1  for f in /root/logic/weekly/*.sh; do bash "$f" -H ;done >> /root/logs/weekly.log 2>&1"
echo
